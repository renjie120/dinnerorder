...
<body>
	...
	<table id='regiestTable'>
		<tr>
			<td>系统名</td>
			<td><input id="registerSystem" /><span>描述:</span><input
				id="desc" /><span class="set" onClick="addSystem(this)"></span></td>
		</tr>
		<tr>
			<td>表名</td>
			<td><select name="system" id="sel1"><option value="-1">请选择系统</option>
					<%
					...
					%></select> <input id="registerTable" />描述:<input id="desc" /><span
				class="set" onclick="addTable(this)"></span></td>
		</tr>
		<tr>
			<td>列名</td>
			<td><select id="sel2" name="system"
				onchange="changeSystem(this)"><option value="-1">请选择系统</option>
					<%
						if (allSystem != null && allSystem.size() > 0) {
							for (Menu m : allSystem) {
					%>
					...
					%></select> <select id="sel3" name="table"><option value="-1">请选择表名</option></select>
				<input id="registerColumn" /> 描述:<input id="desc" /><span
				class="set" onclick="addColumn(this)"></span></td>
		</tr>
		<tr>
			<td>设置列内容</td>
			<td><select id="sel4" name="system"
				onchange="changeSystem(this)"><option value="-1">请选择系统</option><%
						if (allSystem != null && allSystem.size() > 0) {
							for (Menu m : allSystem) {
					%>
