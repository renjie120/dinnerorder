...
(function($) {  
    $.addGt = function(t, p) {   
        ...
        p = $.extend({
            ...
            title: null, //表格树标题
            exchangeColor: true,
            closeImg: '../js/gridTree/images/minus.gif',
            openImg: '../js/gridTree/images/plus.gif',
            lazyLoadImg: '../js/gridTree/images/plus2.gif',
            blankImg: '../js/gridTree/images/blank.gif',
            noparentImg: '../js/gridTree/images/tv-item.gif',
            lastLeafImg: '../js/gridTree/images/tv-item-last.gif',
            morePageImg: '../js/gridTree/images/morePage.gif',
            pageBarId: 'pageBarTable',
            countCellWd: 100,
			noTitle:false,//是否不显示标题栏
            pageSize: 10, 
            pageBar: false,
            lazyPage: false,
            lazyMorePage: false,
            ...
            handleCheck: null,
			attachSth:null,//附加一些参数到表格对象上面.
            data: null, 
			dynamicColumn:null,//动态加载数据时可以根据这个属性设置是否是文件夹!前提：必须有data
			setLazyArguments:null,//超级表格的时候需要传递到后台的行对象里面的参数数组,前提必须设置data和dynamicColumn!函数返回对象类型，将要传递到后台的参数以对象形式返回
            lazy: false,
			setHeader:null,//设置表格的标题的函数
            lazyLoadUrl: null,
            handler: null,
            leafColumn: null,
            rowCountOption: 1,
        ...
        var g = {
            ...
            _resetBasePageBtns: function() {
                ...
                if (pagingInfo.pagesCount == 1 || pagingInfo.pagesCount == 0) {
                    if (pagingInfo.pagesCount == 0) {
                        $('#_toPageBtn',elct).removeClass("nextPage").addClass("disFirstPage").attr("disabled", true); 
                    } 
                } 
            },
            ...
            _makeTable: function(data) {  
                ...
                if (!hasAdded) {
                    if (isthere != 0) $('#_changePageSizeSel option:eq(' + isthere + ')',elct).after("<option value='" + p.pageSize + "' selected='selected'>" + p.pageSize + "</option>");
                    else {
                        $('#_changePageSizeSel',elct).prepend("<option value='" + p.pageSize + "' selected='selected'>" + p.pageSize + "</option>");
            ...
            _getTree: function() {  
                ...
                $(tree,elct).attr('id', "" + p.tableId).addClass('tableRegion'); 
                if (p.pageBar != true) {  
            ...
            _reload: function() { 
                ...
                $.ajax({
                    ...
                    success: function(msg) {
                        ...
                        g._showMsg(0);
                        g._setLastLeaf();
                        mcel.html(["当前第", pagingInfo.currentPage, "页/总共", pagingInfo.pagesCount, "页"].join(''));
                        var gotime = new Date() - o;
            ...
            dynamicAdd: function(parentId, parentNode, imgNode, childNodeStrs, prebotherindex) {   
				...
				var __row_num = parentNode.getAttribute('rownum')==null?parentNode.getAttribute('_node_num'):parentNode.getAttribute('rownum');
                ...
                for (var i = 0; i <= datas.length - 1; i++) {  
                    ...
                    if (p.lazyPage && (i == datas.length - 1) && !p._dynamicAddByUse) {  
                        ...
                        $('tr[_lazy_paging_pId=' + parentId + ']:last img:eq(' + (level - 2) + ')',elct).attr('src', p.blankImg);
                        $('tr[_lazy_paging_pId=' + parentId + ']',elct).removeAttr('_lazy_paging_pId');
                        $('#_lazypagingimg_' + parentId,elct).removeAttr('id').unbind('mouseover').unbind('click').unbind('mouseout');
                        newRow.setAttribute('_lazy_paging_pId', parentId);
                ...
                if (p.lazyPage || (p.lazyMorePage && p._dynamicAddByUse)) {
                    ...
                    if ($('tr[_node_parent=_node' + parentId + ']',elct).size() > 0) {
                        //$('tr[_node_parent=_node' + parentId + ']:last',elct).after(temp);
			...
			openChildrenTable: function(imgid, node, e) { 
				g._d('456--openChildrenTable()---调用打开节点方法');
				...
				if(p.lazy||g.superGrid) {  
					...
					if(p.lazy){
                      param = {  pId: _id  }; 
						...
						if($.isFunction(p.setLazyArguments))
							param = p.setLazyArguments(node,elct); 
						else
							param = {  pId: _id  }; 
					} 
                    ...
                    if ( pN.attr('_expaned') != 'true') { 
                        ...
                        $.ajax({
							...
							$('tr[_node_parent=_node' + _id + ']',elct).show();  
							 $(img,elct).unbind('click').attr('src',p.closeImg).bind('click',function(ee){
                    ...
                    $('tr[_node_parent=_node' + _id + ']',elct).each(function(i,ee) {
						var $this = $(this);
			...
			_hideByPath:function(ppth,e){ 
				$('tr[_node_path^=' + ppth + ']',elct).each(function(){
					...
					if($this.attr('_node_isparent')=='1'){  
						$('#img_'+id,elct).attr('src',p.openImg).unbind('click').bind('click',function(ee){ 
			...
			_newHideByPath:function(node,id){  
				 $(node).nextAll('tr').each(function(){ 
					...
					if($this.attr('_node_isparent')=='1'){  
						$('#img_'+_id,elct).attr('src',p.openImg).unbind('click').bind('click',function(ee){ 
            ...
            select: function(nodeId) {
                $('input[name=_chks][value=' + nodeId + ']:not([userSetDisabled=disabled])',elct).attr('checked', true);
                ...
                if (!p.lazy) { 
					...
					$('tr[_node_isparent=1]',elct).each(function(){
						...
						_$this.find('img[id=img_'+id+']').attr('src',p.closeImg).unbind('click').bind('click',function(e) { 
							g.closeChildrenTable('img_' + id,_$this,e) ;
						}); 
            ...
            _postGetTable: function(u, p) {
                $.ajax({
                    ...
                    error: function(data) {  
						if(p.cnsoleJsonStr)
							  data.responseText = p.cnsoleJsonStr(data.responseText);
                    ...
                    for (__i = 0; __i < allLeaf.length; __i++) {  
						...
						if(src){  
							if(src.indexOf(p.minNoparentImg)!=-1){ 
								$('#' + _lastLeafId,elct).attr('src', p.lastLeafImg);  
							}
						} 
                    }
            ...
            _chooseParentNode: function(checkboxDom) {
                ...
                if (checkboxDom.checked) {
                    while (1) {
                        var parentId = $('#_node' + nodeId,elct).attr('_node_parent');
			...
			_beforeAjax:function(){ 
				 ...
				 var _d = $('#_centerMsg',elct);
                    ...
                    $('#showmsg',elct).width(_bd.width()).height(_bd.height()).css({
                        'top': 0,
                        'left': 0
                    }).show();
				}else{
            ...
            _showLazyPagingDiv: function(pid, imgId, v, way) { 
                ...
                if (v) {
                    ...
                    $('[id^=_lazyPagingDiv_]',elct).hide();
                    $('#_lazy_page_count_' + pid,elct).text(lazypaging_info.allCount);
                    g._resetLazyPageBtns(pid);
                    if (way == 1) {
						lazyPageDiv.css('top',offset.top); 
                        $('#_lazy_page_currentpage_' + pid,elct).text(lazypaging_info.currentPage);
						...
						lazyPageDiv.css('top',offset.top - $('#' + imgId).height()); 
                        $('#_lazy_page_currentpage_' + pid,elct).text(lastcount);
            ...
            _createMoreLazyPagingDiv: function(pid) {
                ...
                var lazypaging_info = _lazypgInfoMap.get(pid);
                $('#_lazy_page_count_' + pid,elct).text(lazypaging_info.allCount);
                $('#_lazy_page_currentpage_' + pid,elct).text(lazypaging_info.currentPage * lazypaging_info.pageSize);
            ...
            _addTitleHead: function() {
                ...
                for (var ii = 0; ii < lenlen; ii++) {
                    ...
                    if (p.columnModel[i].width != null && p.columnModel[i].width != '*'){
						if($.isFunction(p.setHeader))
							jcdiv.width(p.columnModel[i].width).html(p.setHeader(cms[ii],elct)); 
						else
            ...
            _reMakeTable: function(pageSize) {
                if (pagingInfo.pagesCount != 0) {
                    if (_serverPagingMode == 'client') {
                        ...
                        pagingInfo.pagesCount = Math.ceil(pagingInfo.allCount / pagingInfo.pageSize * 1.0);
                        pagingInfo.currentPage = 1;
                        $('#msgCell',elct).html("当前第" + pagingInfo.currentPage + "页/总共" + pagingInfo.pagesCount + "页");
                        g._toPage(1);
            ...
            _addOneRowByData: function(tableObj, index, rowData, level) {
			     g._d(1463+"----添加节点...  _addOneRowByData() rowData.- p.dynamicColumn="+rowData[p.dynamicColumn])
                ...
                if (p.handler) {
                    ...
                    if (!_usHandler.length) {
                        ...
                        if (findInArray(_usHandler, 'onclick') == -1) {
							$row.click(function() { 
								$('tr[id='+_lastSelectRowId+"]",elct).removeClass("selectlight"); 
		...
		if(!p.title)
			p.title = '表格树版本2.2   作者:renjie120';
		if(!p.noTitle)
		   g._tool_Div = $("<div  id='_tool_Div' class='gridTitleBar' ></div>").html(p.title);//.append(testButton2);  
	...
	$.fn.tableObj = function() { 
        return this.each(function() {
            if (this.gt) this.gt._getTable();
